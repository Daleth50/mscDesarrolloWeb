...
Ejecutar: python backend/app/scripts/dev.py
"""
...
{Colors.GREEN}✓ Flask backend{Colors.RESET}: http://localhost:{flask_port}
...
def main():
    ...
    try:
        ...
        flask_process = run_command(
            f"{sys.executable} {backend_dir / 'run.py'}",
            "Flask",
            cwd=backend_dir
        )
        if flask_process:
            processes.append(("Flask", flask_process))
            print(f"{Colors.GREEN}✓ Flask iniciado en http://localhost:{flask_port}{Colors.RESET}\n")
